...
Updated for 7-layer structure with dense PET substrate
"""
...
def log_message(message, log_file=None, verbose=True):
    ...
    if log_file and log_file != True:  # Check log_file is not a boolean
        try:
...
DEFAULT_LOG_FILE = setup_logging()
...
class GeometryGenerator:
    def __init__(self, model=None, log_dir=None, verbose=True):
        """
        Initialize geometry generator for 7-layer structure
        ...
        # Default crack parameters (nanometers)
        self.crack_width = 100.0      # nm
        self.crack_spacing = 10000.0  # nm
        self.crack_offset = 0.25      # fraction
        self.single_sided = True      # Only model one side
        
        # Layer thicknesses from materials (bottom to top) - all in nm
        # 7-layer structure
        self.h0_dense_pet = materials.get_thickness('h0')    # Layer 0: Dense PET substrate
        self.h1_pet = materials.get_thickness('h1')          # Layer 1: PET layer
        self.h2_adhesion = materials.get_thickness('h2')     # Layer 2: Adhesion promoter
        self.h3_barrier1 = materials.get_thickness('h3')     # Layer 3: Barrier 1 (with cracks)
        self.h4_interlayer = materials.get_thickness('h4')   # Layer 4: Interlayer
        self.h5_barrier2 = materials.get_thickness('h5')     # Layer 5: Barrier 2 (with cracks)
        self.h6_topcoat = materials.get_thickness('h6')      # Layer 6: Top coat
        ...
        self.log(f"  h0_dense_pet: {self.h0_dense_pet:.1f}")
        self.log(f"  h1_pet: {self.h1_pet:.1f}")
        self.log(f"  h2_adhesion: {self.h2_adhesion:.1f}")
        self.log(f"  h3_barrier1: {self.h3_barrier1:.1f}")
        self.log(f"  h4_interlayer: {self.h4_interlayer:.1f}")
        self.log(f"  h5_barrier2: {self.h5_barrier2:.1f}")
        self.log(f"  h6_topcoat: {self.h6_topcoat:.1f}")
        self.log(f"  total_height: {self.total_height:.1f}")
    
    def log(self, message):
        """Convenience method for logging"""
        log_message(message, self.log_file, self.verbose)

    def set_model(self, model):
        """Set the ABAQUS model"""
        self.model = model
        self.log(f"Model set: {model.name if model else None}")
    ...
    def set_parameters(self, crack_width=None, crack_spacing=None, crack_offset=None, 
        ...
        if crack_width is not None:
            ...
            self.log(f"  crack_width: {crack_width:.1f} nm")
        if crack_spacing is not None:
            self.crack_spacing = crack_spacing  # nm
            self.log(f"  crack_spacing: {crack_spacing:.1f} nm ({crack_spacing/1000:.1f} um)")
        if crack_offset is not None:
            self.crack_offset = crack_offset
            self.log(f"  crack_offset: {crack_offset:.2f}")
        if single_sided is not None:
            self.single_sided = single_sided
            self.log(f"  single_sided: {single_sided}")
        if thicknesses is not None:
            for key, value in thicknesses.items():
                setattr(self, key, value)
                self.log(f"  {key}: {value:.1f} nm")
        
        # Recalculate total height
        self.total_height = (self.h0_dense_pet + self.h1_pet + self.h2_adhesion + 
                            self.h3_barrier1 + self.h4_interlayer + 
                            self.h5_barrier2 + self.h6_topcoat)
        self.log(f"  total_height: {self.total_height:.1f} nm")
    ...
    def get_geometry_config(self):
        ...
        config = {
            ...
            'layer_thicknesses': {
                'h0_dense_pet': self.h0_dense_pet,
                'h1_pet': self.h1_pet,
                'h2_adhesion': self.h2_adhesion,
                'h3_barrier1': self.h3_barrier1,
                'h4_interlayer': self.h4_interlayer,
                'h5_barrier2': self.h5_barrier2,
                'h6_topcoat': self.h6_topcoat,
    ...
    def save_geometry_config(self, filepath):
        ...
        self.log(f"Geometry configuration saved to: {filepath}")
    ...
    def create_materials(self):
        ...
        for mat_name, diffusivity in materials.diffusivities.items():
            try:
                ...
                self.log(f"  Created material {mat_name}: D={diffusivity:.1e} nm²/s, S={materials.get_solubility(mat_name):.1e} mol/nm³/Pa")
            except Exception as e:
                self.log(f"  ERROR creating material {mat_name}: {str(e)}")
    ...
    def create_sections(self):
        ...
        for mat_name in materials.diffusivities.keys():
            ...
            try:
                ...
                self.log(f"  Created section: {section_name}")
            except Exception as e:
                self.log(f"  ERROR creating section {section_name}: {str(e)}")
    ...
    def create_unit_cell_geometry(self):
        ...
        self.log(f"  Creating part: {part_name} (width={width:.1f} nm, height={self.total_height:.1f} nm)")
        ...
        try:
            ...
            self.log(f"  Unit cell geometry completed with {len(part.faces)} faces")
            return part
            
        except Exception as e:
            self.log(f"  ERROR creating unit cell geometry: {str(e)}")
            raise
    ...
    def _create_layer_partitions(self, part, width):
        ...
        # 7 layers, so 6 interfaces (no partition needed at top)
        layer_thicknesses = [
            self.h0_dense_pet,    # Dense PET substrate
            self.h1_pet,          # PET layer
            self.h2_adhesion,     # Adhesion promoter
            self.h3_barrier1,     # Barrier 1
            self.h4_interlayer,   # Interlayer
            self.h5_barrier2,     # Barrier 2
            # h6_topcoat is last, no partition needed at top
        ]
        
        for i, thickness in enumerate(layer_thicknesses):
            y_current += thickness
            y_positions.append(y_current)
            self.log(f"    Layer {i} interface at y={y_current:.1f} nm")
        
        # Create horizontal partition lines at layer interfaces
        for i, y_pos in enumerate(y_positions):
            try:
                sketch = self.model.ConstrainedSketch(name=f'layer_interface_{i}', sheetSize=1.0)
                sketch.Line(point1=(0.0, y_pos), point2=(width, y_pos))
                part.PartitionFaceBySketch(faces=part.faces, sketch=sketch)
                self.log(f"    Created layer partition at y={y_pos:.1f} nm")
            except Exception as e:
                self.log(f"    ERROR creating layer partition at y={y_pos:.1f}: {str(e)}")
    ...
    def _create_crack_partitions(self, part, width):
        ...
        # Barrier 1 crack parameters (layer 3)
        if self.h3_barrier1 > 0:
            y1_bottom = self.h0_dense_pet + self.h1_pet + self.h2_adhesion
            y1_top = y1_bottom + self.h3_barrier1
            x1_offset = 0.0  # First barrier has no offset
            self.log(f"    Barrier1: y={y1_bottom:.1f} to {y1_top:.1f} nm, offset={x1_offset:.1f} nm")
            self._create_vertical_crack_partitions(part, width, x1_offset, y1_bottom, y1_top, "Barrier1")
        
        # Barrier 2 crack parameters (layer 5)
        if self.h5_barrier2 > 0:
            y2_bottom = self.h0_dense_pet + self.h1_pet + self.h2_adhesion + self.h3_barrier1 + self.h4_interlayer
            y2_top = y2_bottom + self.h5_barrier2
            x2_offset = self.crack_offset * self.crack_spacing
            self.log(f"    Barrier2: y={y2_bottom:.1f} to {y2_top:.1f} nm, offset={x2_offset:.1f} nm")
    ...
    def _create_vertical_crack_partitions(self, part, width, x_offset, y_bottom, y_top, layer_name):
        ...
        self.log(f"    Creating vertical partitions for {layer_name}")
        
        # Calculate crack boundaries with periodic wrapping
        crack_x1 = x_offset % width
        crack_x2 = (x_offset + self.crack_width) % width
        
        self.log(f"      Crack: x1={crack_x1:.1f}, x2={crack_x2:.1f} nm (width={self.crack_width:.1f} nm)")
        ...
        for i, x_pos in enumerate(crack_positions):
            ...
            if tolerance < x_pos < (width - tolerance):
                try:
                    sketch = self.model.ConstrainedSketch(name=f'{layer_name}_crack_{i}', sheetSize=1.0)
                    sketch.Line(point1=(x_pos, y_bottom), point2=(x_pos, y_top))
                    part.PartitionFaceBySketch(faces=part.faces, sketch=sketch)
                    self.log(f"      Created partition at x={x_pos:.1f} nm")
                except Exception as e:
                    self.log(f"      ERROR creating partition at x={x_pos:.1f}: {str(e)}")
            else:
                self.log(f"      Skipped boundary partition at x={x_pos:.1f} nm")
    ...
    def assign_materials_to_regions(self, part, instance):
        ...
        # Get section names - updated for current material set
        sections = {}
        for mat_name in materials.diffusivities.keys():
            sections[mat_name] = f'{mat_name}_section'
    ...
    def _calculate_layer_boundaries(self):
        """Calculate y-coordinates of layer boundaries for 7-layer structure"""
        boundaries = {}
        y_current = 0.0
        
        # Layer 0: Dense PET substrate
        boundaries['dense_pet'] = (y_current, y_current + self.h0_dense_pet)
        y_current += self.h0_dense_pet
        
        # Layer 1: PET
        boundaries['pet'] = (y_current, y_current + self.h1_pet)
        y_current += self.h1_pet
        
        # Layer 2: Adhesion promoter
        boundaries['adhesion'] = (y_current, y_current + self.h2_adhesion)
        y_current += self.h2_adhesion
        
        # Layer 3: Barrier 1
        boundaries['barrier1'] = (y_current, y_current + self.h3_barrier1)
        y_current += self.h3_barrier1
        
        # Layer 4: Interlayer
        boundaries['interlayer'] = (y_current, y_current + self.h4_interlayer)
        y_current += self.h4_interlayer
        
        # Layer 5: Barrier 2
        boundaries['barrier2'] = (y_current, y_current + self.h5_barrier2)
        y_current += self.h5_barrier2
        
        # Layer 6: Top coat
        boundaries['topcoat'] = (y_current, y_current + self.h6_topcoat)
        
        self.log("Layer boundaries (nm):")
        for name, (y_min, y_max) in boundaries.items():
            self.log(f"  {name}: y={y_min:.1f} to {y_max:.1f}")
    ...
    def _calculate_crack_positions(self):
        ...
        if self.h3_barrier1 > 0:
            ...
            crack_positions['barrier1'] = {
                ...
                'y_range': (y1_bottom, y1_top)
            }
            self.log(f"  Barrier1 crack: x={x1_start:.1f} to {x1_end:.1f} nm")
        ...
        if self.h5_barrier2 > 0:
            ...
            if x2_end < x2_start:
                ...
                crack_positions['barrier2'] = {
                    ...
                    'y_range': (y2_bottom, y2_top)
                }
                self.log(f"  Barrier2 crack (wrapped): x=0 to {x2_end:.1f} and x={x2_start:.1f} to {width:.1f} nm")
            else:
                crack_positions['barrier2'] = {
                    'x_ranges': [(x2_start, x2_end)],
                    'y_range': (y2_bottom, y2_top)
                }
                self.log(f"  Barrier2 crack: x={x2_start:.1f} to {x2_end:.1f} nm")
        
        return crack_positions

    def _assign_materials_with_cracks(self, part, boundaries, crack_positions, sections):
        """Assign materials to faces with crack detection for 7-layer structure"""
        self.log(f"Assigning materials to {len(part.faces)} faces...")
        ...
        for i, face in enumerate(part.faces):
            try:
                ...
                if hasattr(centroid_result, '__len__') and len(centroid_result) > 0:
                    ...
                    # Check each layer (7 layers total)
                    if boundaries['dense_pet'][0] <= y_cent <= boundaries['dense_pet'][1]:
                        material = 'dense_PET'
                        layer = 'dense_pet'
                    
                    elif boundaries['pet'][0] <= y_cent <= boundaries['pet'][1]:
                        material = 'PET'
                        layer = 'pet'
                    ...
                    # Assign material
                    if material and material in sections:
                        region = (face,)
                        ...
                        if 'crack' in layer or 'barrier' in layer:
                            self.log(f"  Face {i} at ({x_cent:.1f},{y_cent:.1f}): {layer} -> {material}")
                    
            except Exception as e:
                self.log(f"  Face {i}: Error - {str(e)}")
        
        # Summary
        self.log("Material assignment summary:")
        for layer, count in sorted(assignment_count.items()):
            self.log(f"  {layer}: {count} faces")
    ...
    def create_mesh(self, part, element_size=None):
        ...
        try:
            ...
            self.log(f"  Element size: {element_size:.1f} nm")
            ...
            self.log(f"  Mesh created: {num_elements} elements, {num_nodes} nodes")
            
        except Exception as e:
            self.log(f"  ERROR creating mesh: {str(e)}")
            raise
    ...
    def validate_geometry(self):
        ...
        if self.crack_width >= self.crack_spacing:
            warnings.append(f"ERROR: Crack width ({self.crack_width:.1f} nm) >= spacing ({self.crack_spacing:.1f} nm)")
            is_valid = False
        
        # Check if crack width is very small
        if self.crack_width < 10:
            warnings.append(f"WARNING: Very small crack width ({self.crack_width:.1f} nm) may cause mesh issues")
        
        # Check aspect ratios
        crack_aspect = self.crack_spacing / self.crack_width
        if crack_aspect > 1000:
            warnings.append(f"WARNING: High aspect ratio ({crack_aspect:.0f}) may affect mesh quality")
        
        # Check layer thicknesses
        min_thickness = min(self.h0_dense_pet, self.h1_pet, self.h2_adhesion, 
                           self.h3_barrier1, self.h4_interlayer, self.h5_barrier2, self.h6_topcoat)
        if min_thickness < 10:
            warnings.append(f"WARNING: Very thin layer ({min_thickness:.1f} nm) detected")
    ...
    def print_geometry_summary(self):
        ...
        self.log(f"Domain: {self.crack_spacing:.1f} x {self.total_height:.1f} nm")
        self.log(f"Crack width: {self.crack_width:.1f} nm")
        self.log(f"Crack spacing: {self.crack_spacing:.1f} nm ({self.crack_spacing/1000:.1f} um)")
        self.log(f"Crack offset: {self.crack_offset:.2%}")
        self.log(f"Crack area fraction: {self.crack_width/self.crack_spacing:.3%}")
        self.log("Layer structure (7 layers):")
        self.log(f"  0. Dense PET: {self.h0_dense_pet:.1f} nm")
        self.log(f"  1. PET: {self.h1_pet:.1f} nm")
        self.log(f"  2. Adhesion: {self.h2_adhesion:.1f} nm")
        self.log(f"  3. Barrier1: {self.h3_barrier1:.1f} nm (cracks)")
        self.log(f"  4. Interlayer: {self.h4_interlayer:.1f} nm")
        self.log(f"  5. Barrier2: {self.h5_barrier2:.1f} nm (cracks)")
        self.log(f"  6. Topcoat: {self.h6_topcoat:.1f} nm")
        self.log("=========================")
    ...
    def check_mesh_quality(self, part):
        ...
        try:
            ...
            self.log(f"  Total elements: {mesh_stats['total_elements']}")
            self.log(f"  Total nodes: {mesh_stats['total_nodes']}")
            if 'element_size' in mesh_stats:
                self.log(f"  Element size (nm): min={mesh_stats['element_size']['min']:.1f}, max={mesh_stats['element_size']['max']:.1f}, avg={mesh_stats['element_size']['avg']:.1f}")
            self.log(f"  Elements in crack regions: {mesh_stats['crack_region_elements']}")
            
            return mesh_stats
            
        except Exception as e:
            self.log(f"Error checking mesh quality: {str(e)}")
            return {}
